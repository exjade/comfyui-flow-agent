...
| Key | Value |
|---|---|
| `FLOW_AGENT_BASE_URL` | The current HTTPS URL printed by `START-FLOW.cmd` |
...
To recover the actual key later without displaying it, double-click `scripts\COPY-API-KEY.cmd`. It reads `SERVER_API_KEY` from the configured local Flow Agent `.env` and copies the value to the clipboard.
...
```bash
curl -fsSL https://raw.githubusercontent.com/exjade/comfyui-flow-agent/main/scripts/INSTALL-RUNPOD.sh | bash
```
