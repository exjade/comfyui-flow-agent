...
| Key | Value |
|---|---|
| `FLOW_AGENT_BASE_URL` | The current HTTPS URL printed by `04-START-FLOW.cmd` |
...
| 4 | `04-START-FLOW.cmd` | Start Flow Agent/ngrok and copy the public URL |
...
Then run steps 2, 3, and 4 in order. Step 3 copies the RunPod terminal command to the clipboard and does not execute it on Windows. The user must still sign in to Google, load the unpacked extension, provide their own ngrok authtoken, and save the two RunPod environment variables.

Start Flow Agent through the desktop shortcut or:

```text
scripts\04-START-FLOW.cmd
```
